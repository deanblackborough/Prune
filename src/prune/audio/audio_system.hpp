...
namespace prune {
  ...
  public:
    ...
    [[nodiscard]] bool load_sound(std::string_view sound_id,
                                  const std::filesystem::path& path,
                                  std::string& error);

    void map_event_to_sound(std::string_view event_id,
  ...
  private:
    ...
    static void audio_callback(void* userdata, std::uint8_t* stream,
                               int length) noexcept;
