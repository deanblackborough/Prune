...
namespace prune {
  ...
  bool AudioSystem::load_sound(std::string_view sound_id,
    ...
    if (SDL_LoadWAV(path.string().c_str(), &source_spec, &source_buffer,
    ...
    if (SDL_BuildAudioCVT(&converter, source_spec.format, source_spec.channels,
                          source_spec.freq, m_device_spec.format,
                          m_device_spec.channels, m_device_spec.freq) < 0) {
      error = SDL_GetError();
    ...
    converter.buf = static_cast<Uint8*>(
        SDL_malloc(source_length * static_cast<Uint32>(converter.len_mult)));
  ...
  void AudioSystem::map_event_to_sound(std::string_view event_id,
                                       std::string_view sound_id) {
    if (event_id.empty() || sound_id.empty()) {
      return;
    }

    m_event_sound_map.insert_or_assign(std::string(event_id),
                                       std::string(sound_id));
  }
  ...
  void AudioSystem::audio_callback(void* userdata, std::uint8_t* stream,
                                   int length) noexcept {
  ...
  void AudioSystem::mix(std::uint8_t* stream, int length) noexcept {
    ...
    for (ActiveVoice& voice : m_voices) {
      ...
      const std::uint32_t remaining =
          static_cast<std::uint32_t>(sound.buffer.size() - voice.position);
      const std::uint32_t bytes_to_mix =
          std::min(static_cast<std::uint32_t>(length), remaining);

      SDL_MixAudioFormat(stream, sound.buffer.data() + voice.position,
                         m_device_spec.format, bytes_to_mix, SDL_MIX_MAXVOLUME);
