...
namespace prune {
  ...
  namespace {
    ...
    bool read_required_bool(const YAML::Node& node, const char* key,
                            bool& out) {
      if (!node[key]) {
        return false;
      }
    ...
    bool read_required_uint(const YAML::Node& node, const char* key,
                            GameObjectId& out) {
      if (!node[key]) {
        return false;
      }
    ...
    bool read_required_float(const YAML::Node& node, const char* key,
                             float& out) {
      if (!node[key]) {
        return false;
      }
    ...
    YAML::Node make_object_node(const GameObject& object) {
      ...
      case RenderType::Sprite:
        node["render"]["sprite"]["sprite_key"] =
            object.render.sprite.sprite_key;
    ...
    bool load_object_size_from_node(const YAML::Node& node, GameObject& object,
                                    std::string& error) {
    ...
    bool load_object_collision_from_node(const YAML::Node& node,
                                         GameObject& object,
                                         std::string& error) {
    ...
    bool load_object_render_from_node(const YAML::Node& node,
                                      GameObject& object, std::string& error) {
      ...
      object.render.z_index =
          render["z_index"] ? render["z_index"].as<int>() : 0;

      switch (object.render.type) {
      case RenderType::Rectangle: {
        const YAML::Node rectangle = render["rectangle"];
        const YAML::Node colour =
            rectangle ? rectangle["colour"] : YAML::Node{};
      ...
      case RenderType::Sprite: {
        ...
        object.render.sprite.flip_x =
            sprite["flip_x"] ? sprite["flip_x"].as<bool>() : false;
        break;
      }
      }
    ...
    bool load_object_editor_from_node(const YAML::Node& node,
                                      GameObject& object, std::string& error) {
    ...
    bool load_object_runtime_from_node(const YAML::Node& node,
                                       GameObject& object, std::string& error) {
      ...
      if (!read_required_bool(runtime, "persistent",
                              object.runtime.persistent)) {
    ...
    bool load_object_from_node(const YAML::Node& node, GameObject& object,
                               std::string& error) {
      if (!node.IsMap()) {
  ...
  bool SceneSerializer::load_from_node(SceneState& state,
    ...
    if (!read_required_bool(options, "highlight_selected",
