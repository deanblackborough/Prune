...
namespace prune {
  ...
  enum class CameraMode { Editor = 0, Game = 1 };
  ...
  public:
    ...
    void update_game_camera(const SceneViewport& viewport,
                            const GameObject* target) noexcept;
    void pan_editor_by_mouse_delta(int delta_x, int delta_y) noexcept;

    [[nodiscard]] Transform screen_to_world(const SceneViewport& viewport,
                                            int screen_x,
                                            int screen_y) const noexcept;
    [[nodiscard]] SDL_Rect
    world_to_screen_rect(const GameObject& object) const noexcept;
