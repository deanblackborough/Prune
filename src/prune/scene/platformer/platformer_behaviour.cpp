...
namespace prune {
  ...
  namespace {
    ...
    [[nodiscard]] bool
    is_active_solid_platformer_block(const GameObject& object) noexcept {
      return object.lifecycle.active && object.collision.solid &&
             platformer_concepts::is_platform(object);
    }

    [[nodiscard]] bool
    is_active_platformer_hazard(const GameObject& object) noexcept {
      return object.lifecycle.active && platformer_concepts::is_hazard(object);
    }

  } // namespace

  void PlatformerBehaviour::update(SceneState& state, SceneCamera& camera,
    ...
    update_player(state, camera, platformer_state, dt, input,
                  keyboard_input_enabled);
  }

  GameObject* PlatformerBehaviour::player_object(
      SceneState& state,
      const PlatformerState& platformer_state) const noexcept {
  ...
  const GameObject* PlatformerBehaviour::player_object(
  ...
  const GameObject* PlatformerBehaviour::player_start_object(
      ...
      const PlatformerState& platformer_state) const noexcept {
    const GameObject* object =
        state.objects.get_by_id(platformer_state.player_start_id);
  ...
  void PlatformerBehaviour::update_player(SceneState& state,
                                          SceneCamera& camera,
    ...
    move_player_axis(state, *player, player->motion.velocity.x * dt, 0.0f,
                     grounded);
  ...
  void PlatformerBehaviour::move_player_axis(SceneState& state,
                                             GameObject& player, float delta_x,
                                             float delta_y,
                                             bool& grounded) const {
    ...
    for (const auto& object : state.objects.objects()) {
      if (object.identity.id == player.identity.id ||
          !is_active_solid_platformer_block(object)) {
        continue;
      }
      ...
      if (delta_x > 0.0f) {
        player.transform.x =
            object.transform.x - static_cast<float>(player.size.width);
        player.motion.velocity.x = 0.0f;
      } else if (delta_x < 0.0f) {
        player.transform.x =
            object.transform.x + static_cast<float>(object.size.width);
        player.motion.velocity.x = 0.0f;
      }

      if (delta_y > 0.0f) {
        player.transform.y =
            object.transform.y - static_cast<float>(player.size.height);
        player.motion.velocity.y = 0.0f;
        grounded = true;
      } else if (delta_y < 0.0f) {
        player.transform.y =
            object.transform.y + static_cast<float>(object.size.height);
        player.motion.velocity.y = 0.0f;
      }
    }
  }
  ...
  bool
  PlatformerBehaviour::touches_hazard(const SceneState& state,
                                      const GameObject& player) const noexcept {
    for (const auto& object : state.objects.objects()) {
  ...
  void PlatformerBehaviour::reset_player(
      SceneState& state, PlatformerState& platformer_state) const noexcept {
    GameObject* player = player_object(state, platformer_state);
    if (!player) {
      return;
    }

    if (const GameObject* start =
            player_start_object(state, platformer_state)) {
