...
namespace prune::simple_shooter_factory {
  ...
  GameObject create_player() {
    ...
    player.render.sprite.sprite_key =
        std::string(k_default_player_simple_shooter_sprite_key);
  ...
  GameObject create_wall() {
    return create_wall(176.0f, 128.0f, k_default_object_size,
                       k_default_object_size, "Wall");
  }

  GameObject create_wall(float x, float y, int width, int height,
                         const char* name) {
    GameObject wall;
  ...
  GameObject create_projectile_from_player(const GameObject& player,
    ...
    projectile.transform.x =
        player_center_x - (static_cast<float>(projectile.size.width) * 0.5f);
    projectile.transform.y =
        player_center_y - (static_cast<float>(projectile.size.height) * 0.5f);
