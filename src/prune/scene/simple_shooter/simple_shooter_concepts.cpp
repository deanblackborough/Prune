...
namespace prune::simple_shooter_concepts {
  ...
  const char* purpose(ObjectKind kind) noexcept {
    ...
    case ObjectKind::Wall:
      return "An authored solid obstacle used for player and projectile "
             "collision.";
    case ObjectKind::Spawn:
  ...
  const char* collision_rule(ObjectKind kind) noexcept {
    ...
    case ObjectKind::Enemy:
      return "Projectile hits reset the enemy. Enemy/player contact is not "
  ...
  ObjectConcept describe(ObjectKind kind) noexcept {
    ...
    case ObjectKind::Player:
      return {"simple_shooter.player",
              ...
              true};
    case ObjectKind::Enemy:
      return {"simple_shooter.enemy",
              "Enemy",
              "A target that moves toward the player and respawns when hit.",
              "Projectile hits reset the enemy. Enemy/player contact is not "
              "handled in this slice.",
              false,
              true,
              true};
    case ObjectKind::Projectile:
      return {"simple_shooter.projectile",
              "Projectile",
              ...
              false};
    case ObjectKind::Wall:
      return {"simple_shooter.wall",
              "Wall",
              "An authored solid obstacle used for player and projectile "
              "collision.",
              ...
              true};
    case ObjectKind::Spawn:
      return {"simple_shooter.enemy_spawn",
              "Enemy Spawn",
  ...
  ObjectConcept describe_object(const GameObject& object) noexcept {
    ...
    if (object.identity.type == GameObjectType::Runtime) {
      ...
      if (is_enemy(object)) {
        ...
        result.purpose =
            "Enemy created by gameplay code while the scene is running.";
      }
    }
