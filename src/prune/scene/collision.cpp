...
namespace prune::collision {
  ...
  bool are_active_overlapping(const GameObject& a,
  ...
  void resolve_against_solids(GameObject& object,
    ...
    for (const auto& other : objects.objects()) {
      if (other.identity.id == object.identity.id || !other.lifecycle.active ||
      ...
      const float overlap_left =
          (object_bounds.x + object_bounds.width) - other_bounds.x;
      const float overlap_right =
          (other_bounds.x + other_bounds.width) - object_bounds.x;
      const float overlap_top =
          (object_bounds.y + object_bounds.height) - other_bounds.y;
      const float overlap_bottom =
          (other_bounds.y + other_bounds.height) - object_bounds.y;

      const float resolve_x =
          (overlap_left < overlap_right) ? -overlap_left : overlap_right;
      const float resolve_y =
