...
namespace prune {
  ...
  void SceneInteraction::update(Scene& scene, SceneState& state,
                                SceneCamera& camera,
                                const GridOptions& grid_options, float dt,
                                const Input& input) {
  ...
  bool SceneInteraction::scene_keyboard_input_enabled(
      const SceneState& state) noexcept {
    return state.viewport.keyboard_input_enabled && state.viewport.has_area();
  }

  bool SceneInteraction::scene_mouse_input_enabled(
      const SceneState& state) noexcept {
    return state.viewport.hovered && state.viewport.has_area();
  }

  void SceneInteraction::update_editor_camera(Scene& scene, SceneState& state,
    ...
    camera.pan_editor_by_mouse_delta(input.mouse_delta_x(),
                                     input.mouse_delta_y());
  }

  void SceneInteraction::handle_scene_click(Scene& scene, SceneState& state,
    ...
    GameObject* picked = pick_object_at_screen(
        scene, state, camera, input.mouse_x(), input.mouse_y());

    const bool shift_down = input.is_key_down(SDL_SCANCODE_LSHIFT) ||
  ...
  void SceneInteraction::handle_object_drag(Scene& scene, SceneState& state,
                                            SceneCamera& camera,
                                            const GridOptions& grid_options,
                                            const Input& input) {
    if (state.drag_state.active) {
      if (!input.is_mouse_button_down(SDL_BUTTON_LEFT) ||
          !state.viewport.has_area()) {
        ...
        case DragMode::MoveObjects: {
          ...
          for (const DragObjectStart& start : state.drag_state.object_starts) {
            ...
            if (before.transform.x == object->transform.x &&
          ...
          if (before_objects.size() == 1) {
            scene.record_editor_command(make_object_command(
                ...
                before_objects.front(), after_objects.front(), "Mouse drag"));
            ...
            scene.record_editor_command(make_multi_object_command(
                ...
                std::to_string(before_objects.size()) +
                    " objects, mouse drag"));
          }
        } break;
        ...
        case DragMode::ScaleObject:
          if (GameObject* object =
                  state.objects.get_by_id(state.drag_state.object_id)) {
            const GameObject& before = state.drag_state.object_start;
            const bool changed = before.transform.x != object->transform.x ||
                                 before.transform.y != object->transform.y ||
            ...
            if (changed) {
              scene.record_editor_command(make_object_command(
                  EditorCommandType::ChangeObjectSize, "Scale object", before,
                  *object, "Mouse drag"));
            }
          }
          break;
      ...
      const Transform mouse_world = camera.screen_to_world(
          state.viewport, input.mouse_x(), input.mouse_y());
      const float delta_x =
      ...
      case DragMode::ScaleObject:
        if (GameObject* object =
          ...
          apply_scale_drag(*object, state.drag_state.object_start,
                           state.drag_state.scale_handle, delta_x, delta_y);
        }
        break;
    ...
    const auto scale_handle = scale_handle_from_scale_tool_at_screen(
        scene, state, camera, input.mouse_x(), input.mouse_y());
    if (scale_handle != editor::tools::transform_gizmo::ScaleHandle::None) {
      ...
      state.drag_state.mouse_start_world = camera.screen_to_world(
          state.viewport, input.mouse_x(), input.mouse_y());
      state.drag_state.scale_handle = scale_handle;
      return;
    }

    const std::vector<GameObjectId> movable_ids =
        movable_objects_from_drag_start_at_screen(
            scene, state, camera, input.mouse_x(), input.mouse_y());
    ...
    if (const GameObject* object =
            state.objects.get_by_id(state.drag_state.object_id)) {
      state.drag_state.object_start = *object;
    }
  }

  void SceneInteraction::handle_keyboard_nudge(Scene& scene, SceneState& state,
                                               const GridOptions& grid_options,
                                               const Input& input) {
    ...
    const bool ctrl_down = input.is_key_down(SDL_SCANCODE_LCTRL) ||
    ...
    int step = grid_options.snap_to_grid ? std::max(1, grid_options.grid_size)
    ...
    if (before_objects.size() == 1) {
      scene.record_editor_command(make_object_command(
          EditorCommandType::MoveObject,
          editor_command_type_label(EditorCommandType::MoveObject),
          before_objects.front(), after_objects.front(), "Keyboard nudge"));
    } else if (!before_objects.empty()) {
      scene.record_editor_command(make_multi_object_command(
          EditorCommandType::MoveObjects,
          editor_command_type_label(EditorCommandType::MoveObjects),
  ...
  void SceneInteraction::handle_delete_duplicate_shortcuts(
      Scene& scene, SceneState& state, const GridOptions& grid_options,
      const Input& input) {
    if (!scene_keyboard_input_enabled(state)) {
      return;
    }
    ...
    if (input.was_key_pressed(SDL_SCANCODE_DELETE) ||
        input.was_key_pressed(SDL_SCANCODE_BACKSPACE)) {
  ...
  GameObject* SceneInteraction::pick_object_at_screen(Scene& scene,
                                                      SceneState& state,
                                                      const SceneCamera& camera,
                                                      int screen_x,
                                                      int screen_y) noexcept {
    const Transform world =
        camera.screen_to_world(state.viewport, screen_x, screen_y);
    ...
    for (auto it = objects.rbegin(); it != objects.rend(); ++it) {
      ...
      if (!object.lifecycle.active || !object.render.visible ||
          !scene.object_is_selectable(object)) {
        continue;
      }
      ...
      const bool inside =
          world.x >= bounds.x && world.x < bounds.x + bounds.width &&
  ...
  editor::tools::transform_gizmo::ScaleHandle
  SceneInteraction::scale_handle_from_scale_tool_at_screen(
      Scene& scene, SceneState& state, const SceneCamera& camera, int screen_x,
      int screen_y) noexcept {
    ...
    if (!state.scene_options.highlight_selected ||
        state.objects.selected_count() != 1) {
      return editor::tools::transform_gizmo::ScaleHandle::None;
    }
    ...
    return editor::tools::transform_gizmo::scale_handle_at_point(
        selected_outline, local_mouse_x, local_mouse_y);
  }

  std::vector<GameObjectId>
  SceneInteraction::movable_objects_from_drag_start_at_screen(
      Scene& scene, SceneState& state, const SceneCamera& camera, int screen_x,
      int screen_y) {
    if (editor_tool_allows_body_move(state.editor_tool)) {
      std::vector<GameObjectId> body_move_ids =
          movable_objects_from_move_tool_at_screen(scene, state, camera,
                                                   screen_x, screen_y);
    ...
    return movable_objects_from_handle_at_screen(scene, state, camera, screen_x,
                                                 screen_y);
  }

  std::vector<GameObjectId>
  SceneInteraction::movable_objects_from_move_tool_at_screen(
      Scene& scene, SceneState& state, const SceneCamera& camera, int screen_x,
      int screen_y) {
    GameObject* picked =
        pick_object_at_screen(scene, state, camera, screen_x, screen_y);
  ...
  std::vector<GameObjectId>
  SceneInteraction::movable_objects_from_handle_at_screen(
      Scene& scene, SceneState& state, const SceneCamera& camera, int screen_x,
      int screen_y) {
    std::vector<GameObjectId> movable_ids;
    ...
    if (state.objects.selected_count() > 1) {
      ...
      const SDL_Rect outline{bounds.x - 6, bounds.y - 6, bounds.w + 12,
                             bounds.h + 12};

      const SDL_Rect move_handle =
          editor::tools::transform_gizmo::move_handle_rect(outline);
      if (!editor::tools::transform_gizmo::contains_point(
              move_handle, local_mouse_x, local_mouse_y)) {
        return movable_ids;
      }
  ...
  bool SceneInteraction::selected_screen_bounds(const SceneState& state,
                                                const SceneCamera& camera,
                                                SDL_Rect& bounds) noexcept {
  ...
  bool SceneInteraction::rect_visible(const SceneViewport& viewport,
                                      const SDL_Rect& rect) noexcept {
    return rect.x + rect.w >= 0 && rect.y + rect.h >= 0 &&
           rect.x < viewport.width && rect.y < viewport.height;
  }

  float SceneInteraction::snap_value_to_grid(const GridOptions& grid_options,
  ...
  void SceneInteraction::snap_object_to_grid(const GridOptions& grid_options,
  ...
  void SceneInteraction::apply_scale_drag(
      GameObject& object, const GameObject& start,
      editor::tools::transform_gizmo::ScaleHandle handle, float delta_x,
    ...
    const float start_bottom =
        start_top + static_cast<float>(start.size.height);
    ...
    const int width = std::clamp(static_cast<int>(std::round(right - left)),
                                 k_min_object_size, k_max_object_size);

    const int height = std::clamp(static_cast<int>(std::round(bottom - top)),
                                  k_min_object_size, k_max_object_size);
