...
namespace prune {
  ...
  CameraState& SceneCamera::state() noexcept { return m_state; }

  const CameraState& SceneCamera::state() const noexcept { return m_state; }

  Camera& SceneCamera::editor() noexcept { return m_state.editor; }

  const Camera& SceneCamera::editor() const noexcept { return m_state.editor; }

  Camera& SceneCamera::game() noexcept { return m_state.game; }

  const Camera& SceneCamera::game() const noexcept { return m_state.game; }
  ...
  void SceneCamera::update_game_camera(const SceneViewport& viewport,
                                       const GameObject* target) noexcept {
    if (!target || !m_state.game_options.follow_target ||
        !viewport.has_area()) {
      return;
    }
    ...
    m_state.game.x =
        target_center_x - ((static_cast<float>(viewport.width) / zoom) * 0.5f);
    m_state.game.y =
        target_center_y - ((static_cast<float>(viewport.height) / zoom) * 0.5f);
  }

  void SceneCamera::pan_editor_by_mouse_delta(int delta_x,
  ...
  Transform SceneCamera::screen_to_world(const SceneViewport& viewport,
                                         int screen_x,
                                         int screen_y) const noexcept {
    ...
    return {camera.x + static_cast<float>(local_x) / zoom,
            camera.y + static_cast<float>(local_y) / zoom};
  }

  SDL_Rect
  SceneCamera::world_to_screen_rect(const GameObject& object) const noexcept {
    const Camera& camera = active();
    const float zoom = std::max(camera.zoom, 0.01f);
    ...
    return SDL_Rect{
        ...
        static_cast<int>(
            std::round(static_cast<float>(object.size.width) * zoom)),
        static_cast<int>(
            std::round(static_cast<float>(object.size.height) * zoom))};
  }
} // namespace prune
