...
namespace prune {
  ...
  App::App(const AppConfig& config) : m_fixed_timestep(config.fixed_timestep) {
    init_sdl();
  ...
  void App::run() {
    ...
    while (m_running) {
      ...
      if (m_scene && m_ui && m_window) {
        ...
        m_ui->build(*m_scene, m_window->renderer(), new_scene_requested,
                    new_scene_type, load_scene_requested, audio_enabled,
                    audio_available);
        ...
        if (load_scene_requested) {
          ...
          if (loaded_scene) {
            ...
            m_scene->on_enter();
            m_ui->set_file_status("Loaded scene from " + scene_file_path,
                                  false);
          } else {
  ...
  void App::init_sdl() {
    ...
    if ((IMG_Init(image_flags) & image_flags) != image_flags) {
      const std::string error =
          std::string("Failed to initialise SDL_image PNG support: ") +
          IMG_GetError();
      SDL_Quit();
  ...
  void App::init_audio() {
    ...
    static_cast<void>(m_audio->load_sound(sound_ids::explosion,
                                          sound_path / "explosion.wav", error));
    ...
    m_audio->map_event_to_sound(scene_events::enemy_destroyed,
                                sound_ids::explosion);
  ...
  void App::init_imgui() {
    ...
    const std::filesystem::path font_path = std::filesystem::current_path() /
                                            "assets" / "fonts" /
                                            "JetBrainsMonoNL-Regular.ttf";

    ImFont* font = io.Fonts->AddFontFromFileTTF(font_path.string().c_str(),
                                                16.0f, &font_config);
    ...
    prune::tooling::apply_editor_theme(
        prune::tooling::EditorTheme::PrunePurple);

    if (!ImGui_ImplSDL2_InitForSDLRenderer(m_window->sdl_window(),
                                           m_window->renderer())) {
    ...
    if (!ImGui_ImplSDLRenderer2_Init(m_window->renderer())) {
      ...
      throw std::runtime_error(
          "Failed to initialise ImGui SDL renderer backend");
    }
  }
