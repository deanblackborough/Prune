...
namespace prune::tooling {
  ...
  bool draw_sprite_picker(const char* label, const char* id,
    ...
    if (ImGui::BeginCombo("##sprite",
                          sprite_combo_preview(selected_sprite_key))) {
    ...
    if (const Sprites* selected_resource =
            find_sprite_resource(selected_sprite_key)) {
      imgui::property_table::text("Sprite Key", selected_resource->key.data());
      imgui::property_table::text_wrapped("Sprite Path",
                                          selected_resource->path.data());
    } else {
      imgui::property_table::text(
          "Sprite Key",
          selected_sprite_key.empty() ? "None" : selected_sprite_key.c_str());
      imgui::property_table::text_wrapped(
          "Sprite Path", "Sprite key is not defined in sprite resources");
    }
