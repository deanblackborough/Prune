...
namespace prune {
  ...
  namespace {
    ...
    [[nodiscard]] float tool_button_width(std::string_view label) {
      ...
      return std::max(k_tool_button_min_width,
                      label_size.x + (style.FramePadding.x * 2.0f));
    }

    [[nodiscard]] ToolPaletteLayout
    editor_tool_palette_layout(Scene& scene, const GridOptions* grid_options) {
      const ImGuiStyle& style = ImGui::GetStyle();
      const std::span<const SceneCreationAction> actions =
          scene.scene_creation_actions();
      ...
      float content_height = ImGui::GetTextLineHeight() + style.ItemSpacing.y +
                             ImGui::GetFrameHeight() +
                             (style.ItemSpacing.y * 2.0f) + 1.0f +
                             ImGui::GetTextLineHeight() + style.ItemSpacing.y +
                             ImGui::GetFrameHeight();

      const SelectedObjectActionAvailability selection_actions =
          selected_object_action_availability(scene);
      layout.show_clone_action =
          selection_actions.can_clone_active && grid_options != nullptr;
      layout.show_delete_action = selection_actions.can_delete_selection;
      layout.has_selection_actions =
          layout.show_clone_action || layout.show_delete_action;
      ...
      if (layout.has_selection_actions) {
        layout.selection_action_columns =
            (layout.show_clone_action && layout.show_delete_action)
                ? std::size_t{2}
                : std::size_t{1};
        layout.selection_action_button_width =
            std::max(tool_button_width("Clone"), tool_button_width("Delete"));

        const float selection_actions_width =
            (layout.selection_action_button_width *
             static_cast<float>(layout.selection_action_columns)) +
            (style.ItemSpacing.x *
             static_cast<float>(layout.selection_action_columns - 1));

        content_width = std::max(content_width, selection_actions_width);
        content_height += (style.ItemSpacing.y * 2.0f) + 1.0f +
                          ImGui::GetTextLineHeight() + style.ItemSpacing.y +
                          ImGui::GetFrameHeight();
      }
      ...
      if (!actions.empty()) {
        ...
        layout.scene_creation_action_columns =
            std::min(k_scene_creation_action_columns, actions.size());
        ...
        for (const SceneCreationAction& action : actions) {
          layout.scene_creation_button_width =
              std::max(layout.scene_creation_button_width,
        ...
        const float scene_creation_width =
            (layout.scene_creation_button_width *
             static_cast<float>(layout.scene_creation_action_columns)) +
            (style.ItemSpacing.x *
             static_cast<float>(layout.scene_creation_action_columns - 1));
      ...
      layout.size = ImVec2((k_tool_palette_padding * 2.0f) + content_width,
                           (k_tool_palette_padding * 2.0f) + content_height);

      return layout;
    }

    [[nodiscard]] bool point_in_rect(const ImVec2& point, const ImVec2& min,
    ...
    void draw_editor_tool_button(Scene& scene, EditorTool tool) {
      ...
      if (ImGui::Button(editor_tool_label(tool),
                        ImVec2(k_tool_button_min_width, 0.0f))) {
    ...
    void draw_scene_creation_action_button(Scene& scene,
    ...
    void draw_editor_tool_palette(Scene& scene, const ToolPaletteLayout& layout,
      ...
      draw_list->AddRectFilled(palette_min, palette_max,
                               ImGui::GetColorU32(ImGuiCol_WindowBg, 0.94f),
                               style.WindowRounding);
      draw_list->AddRect(palette_min, palette_max,
                         ImGui::GetColorU32(ImGuiCol_Border),
                         style.WindowRounding);

      ImGui::SetCursorScreenPos(ImVec2(palette_min.x + k_tool_palette_padding,
      ...
      if (layout.has_selection_actions) {
        ...
        if (layout.show_clone_action && grid_options != nullptr) {
          if (ImGui::Button(
                  "Clone",
                  ImVec2(layout.selection_action_button_width, 0.0f))) {
      ...
      if (layout.has_scene_creation_actions) {
        ...
        for (std::size_t index = 0; index < actions.size(); ++index) {
          ...
          draw_scene_creation_action_button(scene, actions[index],
                                            layout.scene_creation_button_width);
        }
      }
  ...
  Ui::~Ui() { destroy_scene_render_target(); }
  ...
  void Ui::build(Scene& scene, SDL_Renderer* renderer,
                 bool& new_scene_requested, SceneType& new_scene_type,
                 bool& load_scene_requested, bool& audio_enabled,
                 bool audio_available) {
    draw_scene_viewport(scene, renderer);

    if (ImGui::BeginMainMenuBar()) {
      ImGui::Text("%.*s%s", static_cast<int>(scene.scene_name().size()),
                  scene.scene_name().data(), scene.is_dirty() ? " *" : "");
      ImGui::Separator();
      ...
      if (ImGui::BeginMenu("View")) {
        ...
        ImGui::MenuItem(scene.scene_tools_label().data(), nullptr,
                        &m_show_scene_tools);
    ...
    if (m_show_view_grid_options) {
      ...
      if (ImGui::Begin("Options", &m_show_view_grid_options)) {
        ...
        if (context.available()) {
          m_options.draw(scene.get_scene_options(), *context.grid_options,
                         *context.camera);
        } else {
          ImGui::TextUnformatted(
              "This scene does not expose generic world options.");
        }
      }
      ImGui::End();
    }
    ...
    if (m_show_inspector) {
      ...
      if (ImGui::Begin("Inspector", &m_show_inspector)) {
        WorldSceneContext context = scene.world_scene_context();
        if (context.available()) {
          m_inspector.draw(scene, context.camera->active());
        }

        if (GameObject* selected =
                scene.get_object_manager().selected_object()) {
    ...
    if (m_show_stats) {
      ...
      if (ImGui::Begin("Stats", &m_show_stats)) {
        WorldSceneContext context = scene.world_scene_context();
        if (context.available()) {
          m_stats.draw(scene.get_object_manager(), scene.get_viewport(),
                       *context.camera);
        } else {
          ImGui::TextUnformatted(
              "This scene does not expose generic world stats.");
        }
      }
      ImGui::End();
    }
  ...
  void Ui::ensure_scene_render_target(SDL_Renderer* renderer, int width,
                                      int height) {
    width = std::max(1, width);
    height = std::max(1, height);

    if (m_scene_render_target && m_scene_render_target_width == width &&
  ...
  void Ui::draw_scene_viewport(Scene& scene, SDL_Renderer* renderer) {
    ...
    if (!ImGui::Begin("Scene", &m_show_scene_viewport,
    ...
    viewport.keyboard_input_enabled =
        viewport.focused && !io.WantCaptureKeyboard && !io.WantTextInput;

    ensure_scene_render_target(renderer, viewport.width, viewport.height);

    if (m_scene_render_target) {
      const ImVec2 viewport_max{viewport_pos.x + viewport_size.x,
                                viewport_pos.y + viewport_size.y};
      const WorldSceneContext context = scene.world_scene_context();
      const GridOptions* grid_options =
          context.available() ? context.grid_options : nullptr;
      const ToolPaletteLayout tool_palette_layout =
          editor_tool_palette_layout(scene, grid_options);
      const ImVec2 palette_min{viewport_max.x - k_tool_palette_margin -
                                   tool_palette_layout.size.x,
                               viewport_pos.y + k_tool_palette_margin};
      const ImVec2 palette_max{palette_min.x + tool_palette_layout.size.x,
                               palette_min.y + tool_palette_layout.size.y};
      const bool mouse_over_tool_palette =
          point_in_rect(io.MousePos, palette_min, palette_max);
      ...
      if (mouse_over_tool_palette) {
        ...
        viewport_input_hovered = ImGui::IsItemHovered(
      ...
      ImDrawList* draw_list = ImGui::GetWindowDrawList();
      draw_list->AddImage(reinterpret_cast<ImTextureID>(m_scene_render_target),
                          viewport_pos, viewport_max);

      scene.set_viewport(viewport);
      scene.draw_viewport_overlays();
      draw_editor_tool_palette(scene, tool_palette_layout, grid_options,
