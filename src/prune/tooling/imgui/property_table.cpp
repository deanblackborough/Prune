...
namespace prune::tooling::imgui::property_table {
  ...
  void label(const char* text) { begin_row(text); }
  ...
  bool combo(const char* label, const char* id, int& current_item,
    ...
    ImGui::PushID(id);
    const bool changed =
        ImGui::Combo("##value", &current_item, items, items_count);
    ImGui::PopID();
  ...
  bool drag_float(const char* label, const char* id, float& value, float speed,
                  float min, float max, const char* format) {
    begin_row(label);
    return ImGui::DragFloat(id, &value, speed, min, max, format);
  }

  bool drag_int(const char* label, const char* id, int& value, float speed,
                int min, int max) {
    begin_row(label);
