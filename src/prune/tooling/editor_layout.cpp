...
namespace prune::tooling {
  ...
  namespace {
    ...
    constexpr float k_scene_panel_height = 275.0f;
    ...
    [[nodiscard]] LayoutMetrics metrics() {
      ...
      const float right_x =
          work_pos.x + std::max(k_margin, work_size.x - right_width - k_margin);
      ...
      const float inspector_height =
          std::max(k_min_inspector_height,
                   (work_pos.y + work_size.y) - inspector_y - k_margin);

      return LayoutMetrics{work_pos,      work_size,   top_y,
                           right_x,       right_width, outliner_y,
                           scene_panel_y, inspector_y, inspector_height};
    }
    ...
    void set_centered_window_default(const ImVec2 size) {
      ImGui::SetNextWindowPos(centered_position(), ImGuiCond_FirstUseEver,
                              ImVec2(0.5f, 0.5f));
  ...
  void EditorLayout::scene_viewport() {
    ...
    const float scene_width =
        std::max(k_scene_min_width, layout.right_x - scene_x - k_margin);

    const float scene_height =
        std::max(k_scene_min_height, layout.work_size.y - k_margin);

    set_window_default(ImVec2(scene_x, scene_y),
  ...
  void EditorLayout::outliner() {
    ...
    set_window_default(ImVec2(layout.right_x, layout.outliner_y),
  ...
  void EditorLayout::scene_panel() {
    ...
    set_window_default(ImVec2(layout.right_x, layout.scene_panel_y),
  ...
  void EditorLayout::inspector() {
    ...
    set_window_default(ImVec2(layout.right_x, layout.inspector_y),
