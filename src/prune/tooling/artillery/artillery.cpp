    ...
    bool Artillery::draw(ArtilleryState& state)
    {
        ...
        if (tooling::imgui::layout::collapsing_header("Player Aims")) {
            if (tooling::imgui::property_table::begin("##artillery_player_aims")) {
                authored_aim_changed |= tooling::imgui::property_table::slider_float("P1 Angle", "##artillery_p1_angle", state.player_one_aim.angle_degrees, 5.0f, 85.0f, "%.1f");
                authored_aim_changed |= tooling::imgui::property_table::slider_float("P1 Power", "##artillery_p1_power", state.player_one_aim.power, state.options.min_power, state.options.max_power, "%.1f");
                authored_aim_changed |= tooling::imgui::property_table::slider_float("P2 Angle", "##artillery_p2_angle", state.player_two_aim.angle_degrees, 5.0f, 85.0f, "%.1f");
                authored_aim_changed |= tooling::imgui::property_table::slider_float("P2 Power", "##artillery_p2_power", state.player_two_aim.power, state.options.min_power, state.options.max_power, "%.1f");
                tooling::imgui::property_table::end();
            }
        }

        if (tooling::imgui::layout::collapsing_header("Slice Rules")) {
            ImGui::TextWrapped("Two shared controls drive the active tank. Angle and power are stored per player. Firing creates a runtime projectile; terrain and tanks are authored scene objects.");
            ...
            ImGui::BulletText("R resets the round with one of three fixed terrain layouts.");
