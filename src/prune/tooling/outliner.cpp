...
namespace prune {
  ...
  void Outliner::draw(GameObjectManager& objects, const Scene& scene) {
    ImGui::PushStyleColor(ImGuiCol_Button,
                          ImVec4(0.4f, 0.2f, 0.6f, 1.0f)); // Normal state
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                          ImVec4(0.5f, 0.3f, 0.7f, 1.0f)); // Hover state
    ImGui::PushStyleColor(ImGuiCol_ButtonActive,
                          ImVec4(0.3f, 0.1f, 0.5f, 1.0f));
    ...
    ImGui::InputTextWithHint("##object_search", "Search objects...",
                             m_object_search.data(), m_object_search.size());
    ...
    if (ImGui::BeginChild("object_list", ImVec2(0.0f, list_height),
      ...
      for (const auto& object : objects.objects()) {
        ...
        if (!filter.empty() && !contains_case_insensitive(label, filter) &&
  ...
  bool Outliner::contains_case_insensitive(std::string_view text,
