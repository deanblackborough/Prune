...
namespace prune {
  ...
  namespace {
    ...
    [[nodiscard]] float clone_offset(const GridOptions& grid_options) noexcept {
      return static_cast<float>(grid_options.snap_to_grid
  ...
  SelectedObjectActionAvailability
  selected_object_action_availability(Scene& scene) {
    ...
    if (GameObject* active = objects.selected_object()) {
      availability.can_clone_active =
          scene.object_is_editable(*active) && active->editor.cloneable;
    }

    for (const GameObjectId id : objects.selected_ids()) {
      const GameObject* object = objects.get_by_id(id);
      if (!object || !scene.object_is_editable(*object) ||
          !object->editor.deletable) {
        continue;
      }
  ...
  bool clone_active_selected_object(Scene& scene,
    ...
    clone.identity.name = objects.make_unique_name(
        clone.identity.name + " Copy", k_invalid_game_object_id);
    ...
    scene.record_editor_command(
        make_create_object_command(*created, created->identity.name));
    return true;
  }
  ...
  bool delete_selected_objects(Scene& scene) {
    ...
    if (deleted_objects.size() == 1) {
      scene.record_editor_command(make_delete_object_command(
          deleted_objects.front(), deleted_objects.front().identity.name));
    } else {
  ...
  bool nudge_selected_object_render_order(Scene& scene, int delta) {
    ...
    scene.record_editor_command(make_object_command(
        ...
        before, *selected, std::to_string(selected->render.z_index)));
